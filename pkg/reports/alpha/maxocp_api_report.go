...
func NewMaxDashReport(bundlesReport bundles.Report) *MaxDashReport {
	...
	for k, bundles := range isNotOK {
		kinds, channels, bundlesNotMigrated, bundlesMigrated := custom.GetReportValues(bundles, "1.22")
...
func mapPkgsComplyingMaxOcpVersion(
	mapPackagesWithBundles map[string][]custom.BundleDeprecate) map[string][]custom.BundleDeprecate {
	complying := make(map[string][]custom.BundleDeprecate)
...
func hasWrongMaxOcpVersion(bundlesPerPkg []custom.BundleDeprecate) bool {
